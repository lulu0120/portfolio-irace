...
qap_eval_seed <- function(instance_index, duplicate_tick = 0L, seed_shift = 0L) {
  as.integer(
    as.integer(seed_shift) +
      100000L * as.integer(duplicate_tick) +
      as.integer(instance_index)
  )
}
...
make_qap_score_functions <- function(instance_table, param_config, time_limit = 5, solver = NULL) {
  ...
  cost_config <- function(c, i,
    ...
    if (!is.null(duplicate_tick)) {
      seed_val <- qap_eval_seed(
        instance_index = as.integer(i),
        duplicate_tick = as.integer(duplicate_tick),
        seed_shift = if (is.null(seed_shift)) 0L else as.integer(seed_shift)
      )
    } else {
  ...
  scores_configs_on_instance <- function(configs, i, base_seed,
    ...
    scores_u <- vapply(seq_along(cfg_u), function(k) {
      seed_val <- qap_eval_seed(
        instance_index = as.integer(i),
        duplicate_tick = 0L,
        seed_shift = as.integer(base_seed)
      )
