...
generate_configs_numeric_irace_multidim = function(
  ...
  if (length(elite_ids) == 0L) {
    ...
    cold_try = 0L
    while (length(chosen_ids) < Nj && cold_try < max_tries) {
      need = Nj - length(chosen_ids)
      init_mat = md_sample_initial_sobol(
        Nj = max(need * 2L, need + 8L),
        info = info,
        seed_j = as.integer(seed_j + 1000L * cold_try)
      )
      id_new = vapply(seq_len(nrow(init_mat)), function(i) md_state_register(state, init_mat[i, ]), integer(1))
      id_new = setdiff(unique(as.integer(id_new)), as.integer(chosen_ids))
      if (length(id_new) > 0L) {
        take_n = min(need, length(id_new))
        chosen_ids = c(chosen_ids, id_new[seq_len(take_n)])
      }
      cold_try = cold_try + 1L
    }
    if (length(chosen_ids) < Nj) stop("Cannot fill Nj with unique never-seen configs in cold start.")
    return(as.numeric(chosen_ids))
  }
